...
@testset "mrPerf.jl" begin
    # All regressions
    res0_ivw = mr_ivw(data.βy, data.se_βy, data.βx, 0.025)
    res0_egger = mr_egger(data.βy, data.se_βy, data.βx)
    res1 = mr_wald(data1.βy, data1.se_βy, data1.βx, 0.05)
    res2_ivw = mr_ivw(data2.βy, data2.se_βy, data2.βx)
    res2_egger = mr_egger(data2.βy, data2.se_βy, data2.βx, 0.025)
    res3_ivw = mr_ivw(data3.βy, data3.se_βy, data3.βx)
    res3_egger = mr_egger(data3.βy, data3.se_βy, data3.βx)
    res_perfect_ivw = mr_ivw(data_perfect.βy, data_perfect.se_βy, data_perfect.βx)
    res_perfect_egger = mr_egger(data_perfect.βy, data_perfect.se_βy, data_perfect.βx)
